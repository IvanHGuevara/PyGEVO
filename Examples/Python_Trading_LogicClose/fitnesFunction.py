...
def fitnesFunction(phenotype,df,debug=False):
    ...
    for index, r in df[2:].iterrows():
        ...
        buyCloseCondition =logicTrading[3]
        sellCloseCondition=logicTrading[4]
        ...
        if sell and precioPosicion!=0:
            ...
            if (pico_ganancia_sell + presupuesto) < 0:
                ...
                return 0
        #Close Buy
        if buy and buyCloseCondition and not buyCondition:
            # cierra buy
           ...
           # pone posicion
            precioPosicion = 0
            if debug:
                print(str("-") + " -> % GAIN:" + str(ganancia) + " CloseBuy -> Indesicion in " + str(r['Close']))
            buy = False
            r_ant = r
            continue
        #Close Sell
        if sell and sellCloseCondition and not sellCondition:
            # cierra sell
            ...
            precioPosicion = 0
            if debug:
                print(str("-") + " -> % GAIN:" + str(ganancia) + " CloseSell-> Indesicion in " + str(r['Close']))
            sell = False
            r_ant = r
            continue
        #Buy
        if buyCondition and not buy and not sellCondition:
            ...
            if debug:
                print(str(contadorOperaciones) + " -> % GAIN:" + str(ganancia) + " -> BUY in " + str(
                    r['Close']) + " x" + str(leverage))
            r_ant = r
            continue
        #Sell
        if sellCondition and not sell and not buyCondition:
            ...
            contadorOperaciones = contadorOperaciones + 1
            if debug:
                print(str(contadorOperaciones) + " -> % GAIN:" + str(ganancia) + " -> SELL in " + str(
                    r['Close']) + " x" + str(leverage))
            r_ant = r
            continue
