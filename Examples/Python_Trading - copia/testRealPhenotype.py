...
#from binance_f.model.constant import *
import ta
from historicalCripto import HistoricalCripto
import os
import pandas as pd
...
class TestRealPhenotype:
    def __init__(self,symbol="BTCUSDT",interval=6,phenotype=None,lapsoMinutes=5,debug=False,trading=False) -> None:
        ...
        self.file="data_RealTime_Con17USDT"
        self.carpeta="data"
        self.ganancia=0
        self.buy = False
        self.sell = False
...
#861512672
#phenotype = "(10,(greaterThan(r['volume_fi'],r['Volume']) or (greaterThan(r_ant['Close'],r['volatility_dcm']) and smallerThan(r['trend_ichimoku_base'],r_ant['Open']))),(smallerThan(r_ant['Low'],r['volatility_bbl']) and (greaterThan(r['trend_cci'],r['volatility_kcw']) or smallerThan(r_ant['trend_macd'],r['trend_dpo']))))"
#phenotype="(5,(greaterThan(r['volume_fi'],r['Volume']) or (greaterThan(r_ant['Close'],r['volatility_dcm']) and smallerThan(r['trend_ichimoku_base'],r_ant['Open']))),(smallerThan(r_ant['Low'],r['volatility_bbl']) and (greaterThan(r['trend_cci'],r['volatility_kcw']) or smallerThan(r_ant['trend_macd'],r['trend_dpo']))))"
#15 mins
#phenotype="(5,(greaterThan(r['volatility_dcm'],r['momentum_ao']) and (greaterThan(r['trend_vortex_ind_pos'],r['volatility_kcp']) and (smallerThan(r_ant['volatility_dcl'],r['trend_ichimoku_b']) or (greaterThan(r['trend_psar_up'],r['momentum_uo']) and greaterThan(r['trend_kst'],r_ant['volatility_bbw']))))),(smallerThan(r['volume_mfi'],r['trend_ichimoku_conv']) and (greaterThan(r_ant['volume_adi'],r['trend_vortex_ind_neg']) and (smallerThan(r_ant['High'],r['volatility_dcm']) or (greaterThan(r['trend_mass_index'],r['trend_adx_pos']) and (smallerThan(r['volatility_bbw'],r['momentum_stoch']) and (greaterThan(r['trend_ichimoku_conv'],r_ant['trend_ichimoku_b']) or (smallerThan(r_ant['Low'],r_ant['momentum_tsi']) or greaterThan(r_ant['Volume'],r_ant['Open'])))))))))"
#trp=TestRealPhenotype("BTCUSDT",interval=4,phenotype=phenotype,trading=False)

#1 min
phenotype="(5,smallerThan(r['trend_aroon_up'],r['trend_mass_index']),(greaterThan(r['volume_em'],r['volatility_dcw']) and (greaterThan(r['trend_aroon_down'],r['trend_cci']) or (smallerThan(r['volatility_bbw'],r['others_cr']) and smallerThan(r['trend_cci'],r['trend_macd_signal'])))))"
#1932057861
phenotype="(5,smallerThan(r['trend_aroon_up'],r['trend_mass_index']),(greaterThan(r['volume_em'],r_ant['trend_vortex_ind_diff']) and (greaterThan(r['trend_aroon_down'],r['trend_cci']) or (greaterThan(r_ant['momentum_stoch_signal'],r['volatility_bbp']) and smallerThan(r['trend_cci'],r['trend_dpo'])))))"
#4187929058
phenotype="(5,smallerThan(r['trend_aroon_up'],r['trend_mass_index']),(greaterThan(r['volume_em'],r_ant['momentum_stoch_rsi']) and (greaterThan(r['trend_aroon_down'],r['trend_cci']) or (smallerThan(r['volatility_bbw'],r['trend_kst_sig']) and smallerThan(r['trend_ema_fast'],r['trend_psar_down'])))))"
#123670227  5x
phenotype="(5,(greaterThan(r['volume_adi'],r['volume_em']) or (greaterThan(r_ant['momentum_ao'],r['volatility_dch']) and (greaterThan(r['trend_ichimoku_conv'],r['trend_visual_ichimoku_a']) and (smallerThan(r['trend_psar_down'],r['volume_obv']) and smallerThan(r['momentum_stoch_signal'],r['trend_ichimoku_a']))))),greaterThan(r['volume_em'],r_ant['volatility_kcli']))"
#3   143585942557
#phenotype="(5,(greaterThan(r['volume_adi'],r['volume_em']) or (greaterThan(r_ant['momentum_ao'],r['volatility_dch']) and (greaterThan(r['trend_ichimoku_conv'],r['trend_visual_ichimoku_a']) and (smallerThan(r['trend_psar_down'],r['volume_obv']) and greaterThan(r['trend_macd_diff'],r_ant['volatility_kcw']))))),greaterThan(r['volume_em'],r_ant['volatility_kcli']))"

#entrenado en 15m 1027798284 pero puesto en 5m  porque dio 292554.2646229507-
phenotype="(5,smallerThan(r['volume_em'],r['volume_sma_em']),(greaterThan(r['volume_adi'],r['trend_vortex_ind_diff']) and (greaterThan(r_ant['volatility_dcm'],r['volatility_dch']) or (smallerThan(r['volume_adi'],r['volume_em']) and (greaterThan(r['trend_kst'],r['momentum_tsi']) or greaterThan(r_ant['trend_cci'],r['trend_kst_sig']))))))"
...
#phenotype de 15 en 5   37375742046
p="(5,smallerThan(r['volume_em'],r['volume_sma_em']),(greaterThan(r['trend_aroon_down'],r['momentum_ppo']) and (greaterThan(r['volatility_kch'],r['volatility_dch']) or (smallerThan(r_ant['volume_adi'],r['volume_em']) and (greaterThan(r['trend_kst'],r['momentum_tsi']) or greaterThan(r['Low'],r_ant['trend_psar_down']))))))"

trp=TestRealPhenotype("BTCUSDT",interval=3,phenotype=phenotype,lapsoMinutes=5,trading=True)
